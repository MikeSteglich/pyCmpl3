...
class CmplGridScheduler(object):
	...
	def __init__(self, port = None ):
		...
		try:
			...
			raise CmplServerException( "Cannot read CmplServer option file <"+self.__optFileName+"> " + e.strerror )
	...
	def getServerId(self, port, maxProblems, performanceIndex, solvers, compatibility=0):
		...
		if type(port)!=int:
			...
			try:
				self.__serverList[id].setCmplServer( xmlrpc.client.ServerProxy( self.__serverList[id].cmplServerUrl , allow_none=False) )
			except:
	...
	def stopServer(self):
		if  self.__server.client_address[0] == "127.0.0.1":
			...
			while len( self.__serverList) >0:
				...
				id = list(self.__serverList.keys())[0] 
