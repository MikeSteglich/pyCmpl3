...
class CmplServerHandler():
	...
	def status(self):
		...
		ret=None
		try:
			cmplServer = xmlrpc.client.ServerProxy(url)
...
class CMPLServerUI(Form):
	...
	#*************** constructor *****************************
  	def __init__(self):
		...
		try:
			...
			MessageBox.Show( "Cannot read CmplServer log file <"+self.__logFile+"> " + e.strerror )
			sys.exit()
		...
		_thread.start_new_thread(self.storeOutput, (0,) )
		_thread.start_new_thread(self.storeOutput, (1,) )
	...
	def readOptionFile(self):
		...
		try:	
			...
			MessageBox.Show("Cannot read CmplServer option file <"+self.__optFileName+"> " + e.strerror  )
			sys.exit()
	#*************** end readOptionFile ************************
	...
	def writeOptionFile(self):
		...
		try:	
			...
			MessageBox.Show("Cannot write CmplServer option file <"+self.__optFileName+"> " + e.strerror  )
			sys.exit()
	#*************** end writeOptionFile ***********************	
